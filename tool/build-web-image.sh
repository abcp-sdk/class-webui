#!/usr/bin/env bash
# Build + push the CLASS webui (Svelte + shadcn) static-host image. Mirrors the
# abcp-sdk webui tool/build-web-image.sh: vite build -> caddy static image ->
# forgejo OCI. The class stack is self-contained, so NAME defaults to
# `class-webui` (never overwrites the standalone `agent-webui`).
set -euo pipefail
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REGISTRY="${REGISTRY:-git.agent.svc.cluster.local}"
NAMESPACE="${NAMESPACE:-abcp}"
NAME="${NAME:-class-webui}"
...
echo "Building webui image -> ${DEST} (buildkitd=${BUILDKIT})"
